...
namespace FogonParillero.Controllers
{
    public class PagoController : Controller
    {
        ...
        // Acción POST para eliminar un pedido
        [HttpPost]
        public IActionResult EliminarPedido(int pedidoId)
        {
            try
            {
                // Buscar el pedido y sus detalles
                var pedido = _context.Pedido.Include(p => p.PedidoDetalles).FirstOrDefault(p => p.PedidoId == pedidoId);
                if (pedido == null)
                {
                ...
                _context.PedidoDetalle.RemoveRange(pedido.PedidoDetalles);
